...
<div className={`flex flex-col items-center text-center space-y-4 p-6 ${props.containerStyles || ''}`}>
  ...
  {props.showButton !== false && (
    <button className={`bg-[#FB4C0A] py-2 px-10 md:px-12 md:py-3 lg:py-3 lg:px-15 rounded-lg text-white font-bold shadow-[0_8px_12px_#FB4C0A] transition-transform hover:scale-110 ${props.buttonStyles || ''}`}>
      {props.buttonTitle}
    </button>
  )}
  {props.showPrice && props.price && (
    <p className={`text-center text-lg lg:text-xl font-semibold ${props.priceStyles || ''}`}>
      ${props.price}
    </p>
  )}
</div>
